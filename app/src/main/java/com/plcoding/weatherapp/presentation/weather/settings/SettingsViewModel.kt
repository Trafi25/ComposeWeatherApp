...
class SettingsViewModel
    ...
    constructor(
        ...
        private fun toggleTemperatureUnit() {
            val next =
                if (_settings.value.temperatureUnit == TemperatureUnit.Celsius) {
        ...
        private fun toggleWindSpeedUnit() =
            toggle(WindSpeedUnit.entries, _settings.value.windSpeedUnit) {
                settingsUseCases.setWindSpeedUnit(it)
            }

        private fun togglePressureUnit() =
            toggle(PressureUnit.entries, _settings.value.pressureUnit) {
                settingsUseCases.setPressureUnit(it)
            }

        private fun togglePrecipitationUnit() =
            toggle(PrecipitationUnit.entries, _settings.value.precipitationUnit) {
                settingsUseCases.setPrecipitationUnit(it)
            }

        private fun toggleTimeFormat() =
            toggle(AppTimeFormat.entries, _settings.value.timeFormat) {
                settingsUseCases.setTimeFormat(it)
            }

        private fun toggleThemeMode() =
            toggle(AppThemeMode.entries, _settings.value.themeMode) {
